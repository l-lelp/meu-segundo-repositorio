// O Google Photos utiliza reconhecimento de imagens para criar tags para as suas fotos e permitir que você busque e encontre fotos de cachorro quando você pesquisa por "cachorro" no campo de busca. Ele utiliza machine learning e outros recursos computacionais avançados para criar essas tags. Pois é, adeus Tuber e bem-vindo ao time de engenharia do Google! Agora você terá que criar uma função no arquivo pratica-16.js que receba dois parâmetros: • a lista de fotos • o termo de busca Com os dados de entrada fornecidos, retorne os itens que correspondem a busca.
...
let lista = [{
  ...
  const buscarFotos = (listaFotos, termoDeBusca) => 
  listaFotos.filter((item) => 
  
  item.tags.includes(termoDeBusca));

  console.log(buscarFotos(lista, "cachorro"));